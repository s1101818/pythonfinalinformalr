...
class Runner(object):
    ...
    def __init__(self, config: Dict):
        ...
        #3self.use_wandb = self.all_args.use_wandb
        ...
        if not self.use_render:
            #3if self.use_wandb:
                #3self.save_dir = str(wandb.run.dir)
                #3self.run_dir = str(wandb.run.dir)
            #3else:
    ...
    def log_train(self, train_infos: Dict, total_num_steps: int):
        ...
        for k, v in train_infos.items():
            #3if self.use_wandb:
                #3wandb.log({k: v}, step=total_num_steps)
            #3else:
    ...
    def log_env(self, env_infos: Dict, total_num_steps: int):
        ...
        for k, v in env_infos.items():
            if len(v) > 0:
                #3if self.use_wandb:
                    #3wandb.log({k: np.mean(v)}, step=total_num_steps)
                #3else:
