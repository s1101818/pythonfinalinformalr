...
class GMPERunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            if episode % self.log_interval == 0:
                ...
                #add1
                #self.print_monitoring_stats(episode)
    ...
    #add1
    def print_monitoring_stats(self, episode):
        """單獨打印監測統計數據"""
        if episode % self.log_interval == 0:
            print("=== 監測統計數據 ===")
            
            actor_stats = self.trainer.policy.actor.log_stats(episode)
        
        if actor_stats:  # 如果有統計數據
